...
| E5.1/ENH-001 | Web/Feed | Enhancement | Server-side `?mood=` ranking — page already reads the URL param and routes correctly (and `<MoodSelector>` writes it), but `getHomeFeedSections()` doesn't pass the mood to the underlying API. Real ranking would extend the feed endpoints with `?mood=<id>` and rank by `facets.mood` overlap. Documented in E5.1 plan §5 Decision and tracking deviations. | `OPEN` | P3 |
