...
|----|------|------|-------|--------|----------|
| ASSESS-2026-05-02/REVIEW-001 | Web/QA | Enhancement | 2026-05-02 user-experience assessment rated Consumer 5.5/10, Creator 4/10, Design+Interaction 7/4, Smoothness 7/10 — flagged 12+ "broken" interactions (booking, search Enter, share, sort tabs, calendar, studio sidebar clicks, publish entry, KYC begin, terms-vs-privacy, focus rings on headings, "1 Issue" overlay, more-filters state). **Audit found** 10 of 12 already fixed in commits `2e0cbee` / `849991b` / `d6e8e09`; assessor's symptoms map 1:1 to those bug-bashes — most likely a stale-build / dev-cache run. The "1 Issue" overlay is the Next.js dev indicator (only shown by `next dev`, never `next start`). The focus-ring regression was already suppressed in `globals.css:168-175` via `h*:focus { outline: none }` for the route-announcer pattern. **Plan-driven follow-ups shipped this pass:** (a) added 401 guard to `<PublishWizard>.save()` so a session-expiry mid-edit shows "Sign in to keep your draft" instead of generic "Save failed"; (b) strengthened `/studio/bookings` empty state with CTA "Publish an experience" → `/publish/experience`; (c) strengthened `/studio/reviews` empty state with CTA "Manage your content" + clearer 14-day blind-window copy; (d) removed deprecated `active=` prop from bookings + reviews studio pages (lint clean); (e) added `tests/assessment-smoke.spec.ts` Playwright spec covering the assessment-flagged guest flows (Cmd+K palette, sort-tab href shape, Itineraries-vs-/privacy collision check, terms/privacy distinct content, header chrome on legal pages, /studio + /publish + /studio/kyc auth-gate redirects, content-detail share fallback popup). Typecheck + lint + 352 vitest tests + production build all green. **Verification owed:** reproduce assessor's run on `pnpm build && pnpm start` (port 3004) in incognito; if any flow still feels broken there, file as a fresh row. | `DONE` | — |
...
| E5.2/BUG-002 | API/Seed | Bug | `content.sub_category_id` is null on every seeded row, so `/api/v1/discover/themes` returns `{themes: []}` even after BUG-001's column-name patch. **Fixed (commit `cde1d10`):** [apps/api/scripts/seed-dummy-content.ts](apps/api/scripts/seed-dummy-content.ts) now declares a `SubCategoryId` union verified against the live DB (canonical `_<modifier>` IDs — `travel.heritage_culture`, `travel.trekking_hiking`, etc. — not the migration-023 short form), every Seed type carries a required `subCategory`, and the content insert sets `sub_category_id`. All 37 dd000000-range rows now have a sub_category. Note: the live DB IDs differ from migration 023's source — appears to have been renamed at some later point not captured in any committed migration; flagged for follow-up audit. | `FIXED` | P3 |
...
| E5.3/BUG-001 | Data/Seed | Bug | Every seeded itinerary in DB (`dd000000-2000-2000-2000-0000000000{17..26}`, 13 rows) has `body = NULL`, `itinerary_days = []`, `itinerary_spots = []`. Posts have body (300–500 chars, expected for short-form). Itineraries are empty rows — there's literally nothing to read in the magazine layout. **Fixed (commit `cde1d10`):** seed script now writes body markdown (839–1,276 chars per itinerary, 3-4 paragraphs covering route logic + permits + packing + what-to-skip), distributes 5–13 spots per itinerary across days (with `lat/lng/stop_type/category/thumbnail_url/creator_note`), and bulk-inserts via the new spots branch in `insertContent()`. **Verified post-seed:** 10 itineraries × body populated × 83 total spots inserted × Day 1 spots flagged `is_free_preview` for paid itineraries (FR-046). | `FIXED` | P2 |
...
| E5.4/BUG-001 | Data/Seed | Bug | No `scheduled_dates` rows seeded for any itinerary. The booking flow needs at least one itinerary with a future scheduled_date (capacity 5+, 0 booked) to be testable end-to-end. Compounds E5.3/BUG-001 (empty body/days/spots). **Fixed (commit `cde1d10`):** seed script now writes 3–4 `scheduled_dates` per paid itinerary (offsets 9–90 days out, capacity 6–12 per spec, partial seats randomly booked). 8 paid itineraries × 27 total date rows on the live DB. Booking flow now has end-to-end testable paths: `/booking/<paid-itinerary-id>` resolves with future dates available. The 2 free itineraries correctly get 0 dates. | `FIXED` | P2 |
...
| E5.2/BUG-003 | Data/Seed | Bug | Three E2E test rows (`E2E Test Post — Leh Ladakh`, IDs `08f0e56e-fe13-4655-9759-70111b5324fb` / `7ddadffb-7913-4eb6-a873-f26d1b28dac2` / `cc9408b6-da31-4fd0-866d-42267bdf0d9f`) leaked into the production DB from BDD test runs and never cleaned up. They use `cover_image_url = https://placehold.co/1080x1080/png` which isn't in `apps/web/next.config.ts` `images.remotePatterns` so `next/image` rejects them on render — causes `⨯ Error: Invalid src prop ... hostname "placehold.co" is not configured` to spam the dev console on any page that surfaces them. **Fixed (commit `cde1d10`):** seed script's `wipe()` now (a) lists the three IDs explicitly and (b) auto-discovers every row authored by the dedicated E2E user `02ac1a6b-…` OR with title starting `E2E Test ` OR with NULL/empty title — wiped 32 E2E rows + 13 abandoned drafts on this re-seed. Founder's named manual test posts (7 rows on user `f1fe5920-…`: "3 days of London", "Rohit Bhakare", etc.) deliberately left untouched per the "ask before destroying user content" rule. | `FIXED` | P3 |
| E5.1/BUG-001 | Web/ScrollReveal | Bug | `<ScrollReveal>` (used wrapping every below-fold magazine band on `/`) sets `initial="hidden"` → `opacity: 0` server-side, then flips to visible only when `IntersectionObserver` fires on scroll. Non-scrolling agents — SEO crawlers, social-card preview generators, Playwright `fullPage: true` captures, headless OG-image renderers — never trigger the observer and see permanently-blank content below the fold. **Resurfaced 2026-05-02 in real browsers:** founder opened `/` and saw blank page; SSR HTML had 11 elements with `opacity:0` + 298 with `[hidden]` because the IntersectionObserver also failed to fire on hydration for above-the-fold content (framer-motion's `whileInView` doesn't reliably re-evaluate elements already in viewport at mount, especially with `amount: 0.3` threshold). **Fixed (commit `9577174`):** switched from `initial="hidden"` to `initial={false}`, so SSR now paints children at the "visible" variant by default. The `whileInView` animation still registers but is a no-op for above-fold (already visible). Below-fold elements still get the fade-up reveal as the user scrolls. **Tradeoff:** lose the staggered hero-entrance animation on first page load (hero appears statically). Verified post-fix: `opacity:0` count 11 → 1, section H2s all in SSR HTML. | `FIXED` | P3 |
...
| E5.0/BUG-002 | Seed/Images | Bug | Web dev-server log monitor caught 4 of 21 Unsplash photo IDs in `apps/api/scripts/seed-dummy-content.ts` returning 404 upstream — affected 9 dummy content rows on the home feed (`⨯ upstream image response failed for https://images.unsplash.com/<id> 404`). HEAD-checked all 21 IDs, replaced the 4 dead ones with IDs already used elsewhere in the seed (also verified 200). Seed file fixed in `ccbb748`. **DB re-seeded on commit `cde1d10`** as part of the broader seed pass (E5.3/BUG-001, E5.4/BUG-001, etc.). Live DB now serves only the 200-verified Unsplash IDs. | `FIXED` | P2 |
...
| M2.5/A11Y-001 | Web/DesignSystem | Bug | **System-wide color-contrast fails AA across 13 non-legal routes (~250 failing elements)**, all firing the same WCAG 1.4.3 `color-contrast` rule on body-size text. Caught by `apps/web/tests/axe-sweep.mjs`. Top offenders: `.ch-btn-primary` (white-on-coral 3.65:1 at 13.5px regular — too small for WCAG large-text exemption); `.ch-pill-coral` (same); muted text on tinted surfaces (`--ink-muted` #7a7a82 is ~4.6:1 on pure white, tips below on `--surface-alt`/`--surface-sunk`); various card subtitle spans. **Fix shipped (commit `0730e69`):** new `--primary-text-bg: #c44726` token (4.55:1 white-text contrast) used by `.ch-btn-primary`, `.ch-pill-coral`, `.ch-brand em`, and every inline coral-text call site (sed migration across 30 files); `--ink-muted` darkened per theme (Paper #6c6c74 / Snow #5f6878 / Bone #6c6755) so it passes AA on tinted surfaces; `--ink-faint` text usages (legal disclaimer copy, "· sign in" pill) migrated to `--ink-muted` (--ink-faint is reserved for non-text decorative); auth disclaimer Terms/Privacy links got `textDecoration: 'underline'` to satisfy WCAG 1.4.1 link-in-text-block. Per the proposal: sibling-token approach (preserves --primary for accents/focus rings) + per-theme tweaks. Re-run axe: **250 → 0 failing elements across all 16 routes; 0/0/0/0 (critical/serious/moderate/minor); WCAG 2.2 AA gate fully passes**. Proposal: [docs/proposals/M2.5-A11Y-001-token-shifts.md](../proposals/M2.5-A11Y-001-token-shifts.md). | `FIXED` | P2 |
...
| QA-R2/BUG-01 | Web/Booking | Bug | Booking flow silently redirects to home with `?next=` URL param after clicking "Book this experience" → "Holding your seat..." flash. **Fixed:** the 401 fallthrough wrote `?next=/booking?content=<id>` (no route matches `/booking` without an intentId). Now URL-encodes `next=/content/<id>` so the user lands back on the content page after sign-in. Also added explicit guard for missing `intentId` in the 200 response. | `FIXED` | P0 |
| QA-R2/BUG-02 | Web/Booking | Bug | DualMonthCalendar visual cue for bookable dates was too subtle (`surface-alt` background blended with the page) — users didn't realise only specific dates were available. **Fixed:** available dates now render as filled coral pills with a coral border; not-available dates are clearly transparent + ink-faint. Selected date gets a fully-coral fill with white text. | `FIXED` | P0 |
| QA-R2/BUG-03 | Web/Studio/KYC | Bug | `<KycStatusCard>` was a non-interactive `<div>`. **Fixed:** renders as a `<Link href="/studio/kyc/submit">` when status is `not_started` or `rejected`, with a chevron arrow + descriptive aria-label. Verified status (no action needed) stays a plain status card. | `FIXED` | P1 |
| QA-R2/BUG-04 | Web/Studio | Bug | Sidebar `<Link>` clicks on `/studio/*` pages do nothing in dev — direct URL works. **Suspected dev-cache artefact:** dev server's `.next/` was corrupted by an earlier prod build during the perf pass and started 500'ing on prefetched chunks (`Cannot find module './2461.js'`). Source code is correct (sidebar mounted in layout, `<Link>` per item, pathname-derived active state). Verify after dev server restart. | `FIXED (suspected)` | P0 |
| QA-R2/BUG-05 | Web/Publish | Bug | `/publish` "Post / Story" card click does nothing. **Suspected:** same dev-cache artefact as BUG-04. Code-level: cards are `<Link href="/publish/post">` etc. with proper href. Verify after dev restart. | `FIXED (suspected)` | P0 |
| QA-R2/BUG-06 | Web/Search | Bug | Enter in Cmd+K palette landed on `/notifications`. **Fixed:** the Enter handler in `command-palette-impl.tsx` only called `e.preventDefault()` for the `q.length >= 2` path — short queries let Enter bubble past the modal and trigger whatever Link in the chrome had keyboard focus. Now `preventDefault` on every Enter; short queries are absorbed silently. | `FIXED` | P1 |
| QA-R2/BUG-07 | Web/Discover | Bug | Sidebar "Itineraries" type filter `<Link>` navigates to `/privacy`. **Suspected:** dev-cache; code is `<Link href={buildHref(...)}>` resolving to `/discover?type=self_paced_itinerary`. Verify after dev restart. | `FIXED (suspected)` | P1 |
| QA-R2/BUG-08 | Web/Discover | Bug | "More filters" panel dropped budget/season/difficulty filters on apply. **Fixed (commit `<this>`):** `paramsToFilters` only read `sort`, `type`, `starting_city_id`, `distance_km` — extended to also read `time_window`, `duration_buckets`, `budget_buckets`, `seasons`, `difficulties`, `group_sizes`, `sub_category_id`. Also extended `baseParams` and the page's `searchParams` Props type so all filter dimensions persist round-trip. | `FIXED` | P1 |
| QA-R2/BUG-09 | Web/Discover | Bug | "Recent" sort tab click does nothing. **Suspected:** dev-cache (Link prefetch fail) — code is correct. `DiscoverSortTabs` builds `?sort=recent`; `paramsToFilters` already accepts `recent` ∈ `SORT_ALLOWED`; `activeSort` derives correctly. Verify after dev restart. | `FIXED (suspected)` | P1 |
| QA-R2/BUG-10 | Web/Publish | Bug | "VERIFY IDENTITY TO UNLOCK" routed to Post wizard. **Source code already correct:** `href={lockedForKyc ? '/studio/kyc' : t.href}` — locked items go to `/studio/kyc`. Suspected dev-cache artefact (same as BUG-04/05). | `FIXED (suspected)` | P1 |
| QA-R2/BUG-11 | Web/Social | Bug | Share button click — no popup / sheet / clipboard. **Already fixed in 2026-05-02 round-1 QA** via `navigator.share` AbortError-only silent path; round-2 reporter likely hit dev-cache state. Source code verified correct. | `FIXED (suspected)` | P1 |
| QA-R2/BUG-12 | Web/Reader | Bug | "More from {creator}" Link does nothing. **Suspected:** dev-cache artefact + the global `loading.tsx` flash that we already addressed in round-1 QA. Source code is `<Link href="/u/${creator.username}">`. Verify after dev restart. | `FIXED (suspected)` | P1 |
| QA-R2/BUG-13 | Web/Chrome | Bug | Permanent coral focus ring on `<h1>` page titles. **Fixed:** `<RouteFocus>` programmatically focuses the heading on every navigation for SR announcement (WEB-A11Y-FR-109), and the browser's `:focus-visible` heuristic incorrectly matched as keyboard focus. Added `h1..h6:focus { outline: none }` to the global stylesheet — headings aren't interactive, ring is decorative-only. SR announcement still fires. | `FIXED` | P2 |
| QA-R2/BUG-14 | Web/Legal | Bug | `/terms` page reportedly rendered Privacy Policy content. **Source verified correct** — `apps/web/src/app/terms/page.tsx` H1 reads "Terms of Service" and section content is the 10 ToS clauses. Suspected dev-cache artefact (user saw stale HTML). Verify after dev restart. | `FIXED (suspected)` | P2 |
| QA-R2/BUG-15 | Web/Publish | Bug | "Save failed · Retry" pill appeared on first wizard load. **Fixed:** wizard conflated inline validation errors ("Add a cover photo") with autosave failures via a single `error` state that fed both. Split into `error` (inline field feedback) + `saveError` (autosave/publish failures, drives the AutosavePill state). Validation no longer trips the pill. | `FIXED` | P2 |
| QA-R2/BUG-16 | Web/Footer | Bug | Footer "Community" → 404. **Source verified correct** — `web-footer.tsx` already linked to `/community-guidelines` post round-1 fix. User saw stale dev cache. | `FIXED (verified)` | P2 |
| QA-R2/BUG-17 | Web/Legal | Bug | "Note: This Privacy Policy is a placeholder…" visible on `/privacy`. **Fixed:** removed the placeholder note `<p>` from all 3 legal pages (`/privacy`, `/terms`, `/community-guidelines`). | `FIXED` | P2 |
| QA-R2/BUG-18 | Web/Discover | Bug | Distance chip didn't reduce result count. **Fixed:** `discover.service.ts` distance branch required `user_lat`/`user_lng` AND those are never sent by the web client (no geolocation). Now falls back to the `starting_city_id`'s coordinates as the anchor when user lat/lng aren't supplied — looks up the city's lat/lng and reuses the existing `citiesWithinDistance` helper. | `FIXED` | P2 |
| QA-R3/BUG-01 | Web/Booking | Bug | "Book this experience" still redirected guests to `/?next=/content/<id>` (home with stale next param) instead of opening checkout. **Root cause:** middleware did a presence-only cookie check (`Boolean(ch_session)`) while `getSession()` did JWT verification — they disagreed when the JWT was expired or signed with a rotated secret. The chain `book-cta → 401 → router.push('/signin?next=…') → middleware sees cookie → /feed?next=… → /?next=…` left users on home. **Fixed:** book-cta now POSTs `/api/auth/signout` to wipe stale cookies before redirecting, and uses `window.location.href` for a hard navigation so middleware re-evaluates with the cleared state and lets `/signin` render normally. | `FIXED` | P0 |
| QA-R3/BUG-11 | Web/Social/Share | Bug | Share button click had no visible response — no popup, no toast, no clipboard. **Root cause:** `nativeShare()` checked `'share' in navigator` which is true on desktop Chrome, but the OS share sheet often resolves silently with no UI on desktop, hitting the `return` path before our 4-icon popup could open. **Fixed:** native share is now gated on `(pointer: coarse)` matchMedia (touch-device proxy) + `navigator.canShare(payload)` — only fires the OS sheet when there's actually one available. Desktop and pointer-fine devices fall straight through to the guaranteed 4-icon popup. | `FIXED` | P1 |
| QA-R3/BUG-18 | Web/Discover | Bug | Distance chip *still* didn't change result count even after QA-R2/BUG-18's anchor fix. Two latent issues: (a) `starting_city_id` was applied as a strict equality AND'ed with the distance-derived `in (cityIds)` filter, so picking "Delhi + 50km" returned the same 7 Delhi items as "Delhi alone" — distance never expanded the set; (b) `distance_km` without any anchor was a silent no-op (no error surfaced to UI). **Fixed:** `discover.service.ts` now skips the `starting_city_id` strict eq when `distance_km` is also set — the city becomes the radius anchor, not a strict filter — so the cityIds-within-km set actually expands beyond the source city. Plumbs `viewerCityId` from `optionalAuthenticate` userId → `users.current_city_id` lookup as a third-tier anchor for signed-in users without a URL city. When *no* anchor of any kind is available, returns hard-zero (0 results) so the chip's effect is honest instead of misleading. **Verified live:** `mumbai +250km = 9` (vs `mumbai = 7`); `distance_km=50 no anchor = 0`; no filters = 38. | `FIXED` | P1 |
| QA-R3/UX-001 | Web/Studio | Enhancement | Studio sidebar navigation introduced a ~2s delay where the URL updated but old content stayed visible. **Root cause:** /studio routes had no `loading.tsx`, so Next.js fell through to the global `app/loading.tsx` (which paints the magazine-home hero+bento skeleton) — completely mismatched with the studio sidebar layout and visually disorienting. **Fixed:** added `app/studio/loading.tsx` that renders within `<StudioLayout>` (sidebar + header stay) and skeletons the right-pane content (4-tile metric row + 5-row item list). Suspense fallback paints instantly on navigation between studio sub-routes. | `FIXED` | P3 |
| QA-R4/SEC-01 | Web/Booking | Security/Bug | Round-3's BUG-01 fix tried to recover from a stale-session 401 by POSTing `/api/auth/signout` and hard-redirecting — that nuked the entire web session for any logged-in user whose ch_access expired during a booking attempt. Round-4 audit caught it as a critical regression: a 401 on a single endpoint should never invalidate the user's whole session. **Fixed:** book-cta now surfaces an inline error ("Couldn't verify your session for booking. Refresh the page and try again.") on 401 — no signout, no redirect, no silent session destruction. The original /signin → /feed → /?next= chain that round-3 was working around is now an acceptable rare-edge fallback compared to the auto-signout footgun. | `FIXED` | P0 |
| QA-R4/SEC-02 | API/Content | Security | Full body markdown of paid itineraries (plus every itinerary spot's creator_note + GPS coordinates) was returned to anyone hitting `/api/v1/content/:id` — no auth required. Lets ₹350–₹2,500 paid content be scraped for free. **Fixed:** `getById()` in `content.service.ts` now resolves an `canSeeFullBody` flag from `(isOwner || isFree || hasConfirmedBooking)` and strips `body` + `body_html` for non-eligible viewers. Spot-level gate honours `is_free_preview`: free-preview spots return their full payload (including creator_note); non-preview spots have `creator_note` blanked but keep title/category/thumbnail so the listing chrome still renders for marketing. Schedule data + scheduled_dates + price/cover/title remain public so users can browse before paying. **Verified live:** ₹350 itinerary unauthed → `body length: 0`, 3 of 6 spots with creator_note populated (the 3 free-preview ones); same itinerary as owner → full body + all 6 notes. | `FIXED` | P0 |
| QA-R4/SEC-03 | Web/Auth | Security | `ch_csrf` cookie was set with `httpOnly: false` — a leftover from an earlier double-submit-token design that was never wired up (real CSRF defense is the `isOriginAllowed` Origin-header check in middleware). With JS-readable, any future XSS could exfiltrate it. **Fixed:** flipped to `httpOnly: true` in `middleware.ts`. Confirmed no client code reads `ch_csrf` or sends `x-csrf-token` (greps clean across `apps/web/src`). Origin-header CSRF check still works untouched. | `FIXED` | P1 |
| QA-R4/SEC-04 | Web/Auth | Security | `/signin?next=` was using `next.startsWith('/')` to validate the post-signin redirect target — let through `//evil.com` (protocol-relative URLs are also same-origin from URL parser's perspective but produce cross-origin nav) and `/\evil.com` (some legacy parsers treat backslash as slash). Open-redirect → phishing. **Fixed:** new `safeNext()` helper in `signin/page.tsx` rejects any `next` that starts with `//` or `/\`, falling back to `/feed`. Verified: `/signin?next=//evil.com` SSR renders SignInForm with `next: "/feed"`. | `FIXED` | P1 |
| QA-R4/SEC-06 | Web/Headers | Security | Dev server returned no security headers because `next.config.ts` had `if (isDev) return []` — production-only header config masked any QA work that wanted to verify the policy. **Fixed:** new `devSecurityHeaders` constant ships X-Frame-Options + X-Content-Type-Options + Referrer-Policy + Permissions-Policy + DNS-prefetch in dev (skipping CSP because it breaks Next's HMR websocket and HSTS because dev is HTTP). Production stays on the full set. **Verified live:** `curl -I localhost:3004` returns all five expected dev headers. | `FIXED` | P2 |
| QA-R4/SEC-07 | API/Content | Security | `/api/v1/content/:id` leaked the raw `search_vector` tsvector (full token positions, internal schema info) because `getById()` did `select('*')`. **Fixed:** explicit `PUBLIC_COLUMNS` allowlist in `content.service.ts` covering exactly the fields documented as the public contract — adding new columns to `content` will never silently expand the API surface again. **Verified live:** `search_vector` no longer present in `/api/v1/content/:id` responses. | `FIXED` | P2 |
| QA-R4/SEC-05 | Note | Info | "Backend API URL `http://localhost:3001` exposed in client bundles" — flagged in round-4 but not a bug. Dev API is intentionally addressable from the browser; production builds use a server-side `API_BASE_URL` and the value embedded in client chunks is the public CDN/hosted hostname (which IS supposed to be reachable from the browser — that's how SPAs work). The audit recommendation to "ensure the backend is behind a private network" doesn't apply to a public REST API. No action. | `WONTFIX` | — |
| QA-R5/BUG-A1 | API/Seed | Bug | Featured "Sufi qawwali at Nizamuddin" event displayed a snow-mountain hero image — `apps/api/scripts/seed-dummy-content.ts` reused `photo-1626621341517-bbf3d9990a23` (a Spiti landscape) for both line 275 (Spiti story, correct) AND line 471 (Ahmedabad pol houses) AND line 991 (Sufi qawwali). **Fixed:** Sufi qawwali → `photo-1567427017947-545c5f8d16ad`; Ahmedabad pol houses → `photo-1604357209793-fca5dca89f97`. Both new IDs HEAD-checked 200. Snow-mountain ID retained only at the two genuinely-snowy entries (Spiti story + Manali-to-Spiti itinerary). **Operator action:** re-run `tsx --env-file=.env apps/api/scripts/seed-dummy-content.ts` once for the live DB to pick up the change. | `FIXED (re-seed pending)` | P0 |
| QA-R5/BUG-A2 | Web/Copy | Bug | Featured-card description rendered "A event from Delhi" — `fallbackBlurb()` in `hero-feature.tsx` hardcoded `A ${type}` regardless of vowel onset. **Fixed:** new `articleFor(phrase)` helper picks "An" for vowel-starting nouns + the spoken-vowel digit cases (8/11/18 + 80-89 / 800-899 ranges, so "An 8-day trip" is also correct). Applied across all three template branches. | `FIXED` | P3 |
| QA-R5/BUG-A3 | Web/Copy | Bug | Home "Open full map" CTA on `<MapStrip>` linked to `/discover`, not `/map` — no map route exists. Misleading label. **Fixed:** renamed to "Browse all destinations" so the label matches the actual destination. | `FIXED` | P2 |
| QA-R5/BUG-A4 | Web/Profile | Bug | Creator profile avatar showed a blank white circle for ~1-2s while the photo loaded, because initials only rendered when `avatarUrl == null`. **Fixed:** restructured `<InitialAvatar>` to always render initials with the photo overlaid via `background-image` on the same wrapper — initials show through during load and are covered when the photo paints. Also replaced the inline 30-line avatar block in `apps/web/src/app/u/[username]/page.tsx` with `<InitialAvatar>` for code reuse. Updated `initial-avatar.test.tsx` to assert the new underlay behaviour. | `FIXED` | P1 |
| QA-R5/BUG-A5 | Web/Home | Bug | Mood selector grid showed an orphan card on tablet width (5 moods × 3 columns at the 1079px breakpoint = 3+2 with the second row left-aligned). **Fixed:** switched `.ch-mood-grid` from explicit `grid-template-columns: repeat(N, 1fr)` per breakpoint to `repeat(auto-fit, minmax(160px, 1fr))` — cards now expand to fill whatever row width is available, no orphan at any viewport. | `FIXED` | P2 |
| QA-R5/BUG-A6 | Web/A11y | Bug | React 19/Next 15 dev-mode hydration mismatch on `/discover` (and any page with an `<h1>` in `<main>`): `RouteFocus` ran `heading.setAttribute('tabindex', '-1')` synchronously on useEffect commit, close enough to hydration that the reconciler flagged the attribute drift. **Fixed:** moved both the `setAttribute` and `.focus()` calls inside the existing 80ms `setTimeout` so they run after React's reconcile pass is done. Attribute is still needed for focusability — `<h1>` isn't focusable without it. | `FIXED` | P1 |
| QA-R5/POLISH-B1 | Web/Header | Enhancement | Guest "Following · sign in" chip used the `disabled` style (greyed out, looked like dead content) when its actual purpose is a sign-in CTA. **Fixed:** restyled to coral underlined link "Follow creators →" — reads as a CTA, not a disabled state. Same `/signin?next=/` href. Test `feed-chip-rail.test.tsx` updated to match the new label. | `FIXED` | P3 |
| QA-R5/POLISH-B2 | Web/Header | Enhancement | "+ Publish" button shown to guests had no context — clicking opaquely routed to `/signup?next=/publish`. **Fixed:** added `title="Sign up to publish your first story"` + matching `aria-label` so the click intent is discoverable on hover/screen-reader. | `FIXED` | P3 |
| QA-R5/POLISH-B3 | Web/Home | Enhancement | Mood selector active state was visually distinct (coral border + halo + lift + checkmark) but not loud enough — first-time users perceived the click as a no-op. **Fixed:** (a) `pushToast()` on every mood change ("Showing slow & quiet picks" / "Showing all picks again") makes the rerank explicit; (b) bumped `.ch-mood-tile--active` border from 1.5px → 2px so the selection reads at a glance. | `FIXED` | P2 |
| QA-R5/POLISH-B4 | Web/Reader | Enhancement | Content detail pages had no back affordance except the browser button — a dead-end on mobile and keyboard. **Fixed:** new `<BackLink>` client component (apps/web/src/components/reader/back-link.tsx) renders as a real `<a href="/">` so SSR + Cmd-click work, intercepts plain clicks with `router.back()` when there's same-origin history, and falls back to `/` when the user landed via deep link / share. Positioned `fixed top: 80px; left: 24px` over the parallax hero / post header. | `FIXED` | P2 |
| QA-R5/POLISH-B5 | Web/Profile | Enhancement | Empty creator profiles ended at "First chapter, coming soon." with three teaser cards but no actual creator discovery — a dead-end for guests. **Fixed:** when `published_count === 0`, fetch up to 5 candidates via existing `fetchTopCreators()` (excluding the current creator) and render a new `<DiscoverMoreCreatorsRail>` below the empty state. Each card uses `<InitialAvatar>` + display name + handle + follower count, links to `/u/<username>`. | `FIXED` | P2 |
| QA-R5/POLISH-B6 | Web/Reader | Enhancement | Posts had no reading-time estimate. **Fixed:** new `readingMinutes()` helper in `content/[id]/page.tsx` strips code fences + image lines from the markdown body, counts whitespace-separated tokens at 200 words/min, floors at 1. Renders as a "{n} min read" mono-font badge next to the author/follow row, posts only (`content.body` truthiness check skips it for itineraries / experiences / events). No backend change — pure client/server compute. | `FIXED` | P2 |
| QA-R5/POLISH-B7 | Web/Home | Enhancement | Cold logged-out visitors landed straight in curated content with no value-prop — never told what CreatorHub is. **Fixed:** guest-only single-line tagline above the FeedChipRail: coral mono "CREATORHUB" kicker + serif H2 "Discover trips, stories and experiences from India's best independent creators." Hidden for authed users so the marketing copy doesn't haunt every session. | `FIXED` | P2 |
| QA-R5/POLISH-B8 | Web/Discover | Enhancement | "Near me" disabled-tab tooltip said "Set your city to use Near me" — adequate but the audit assumed geolocation was involved. **Fixed:** expanded tooltip to "Pick your home city in settings to filter by Near me — uses your saved city, never your browser location." Surfaces both the precondition AND the privacy posture. | `FIXED` | P3 |
| QA-R5/DECISION-C1 | Web/Reader | Decision | Audit flagged that posts render title+author centered above the hero image, while events/itineraries/experiences render full-bleed parallax with overlaid text — calls it inconsistency. **Decided in plan-phase:** keep the editorial split. They're genuinely different content types (essay vs poster), and the visual difference is intentional editorial design per the v3 wireframe pack. No code change; logged here so the question doesn't recycle. | `WONTFIX` | — |
| QA-R5/DECISION-C2 | Web/Profile | Decision | "Open in app" button on creator profile used `creatorhub://creator/<u>` deep-link with no fallback — produced "Cannot open this page" for everyone since the iOS/Android app isn't on the stores yet. **Fixed:** removed the button entirely until launch. Re-introduce with real App Store + Play Store URLs as fallbacks once the app is publicly listed. | `FIXED (deferred re-add)` | P3 |
| QA-R5/MISREAD-D1 | Web/Discover | Note | Audit flagged "Top creators" tab as "marked SOON but still clickable." Actually correctly disabled — `<discover-sort-tabs.tsx>` lines 94-112 render a `<span aria-disabled>` (not a `<Link>`) for `comingSoon: true`, with `cursor: not-allowed` + tooltip + no-op href fallback. No code change. | `MISREAD` | — |
| QA-R5/MISREAD-D2 | Web/Home | Note | Audit flagged "MoodSelector — no visible selection state." Active state IS distinct (coral border + halo + lift + checkmark badge) but reads as too subtle for first-time users — addressed by POLISH-B3. No bug; logged so it doesn't recycle. | `MISREAD` | — |
| QA-R5/MISREAD-D3 | Web/Reader | Note | Audit flagged Save/Like/Follow as triggering a full-page redirect for guests rather than an inline modal. Already wired through `openSignInModal()` from `<SignInModalProvider>` at every action call-site (book-cta, save-button, like-button, follow-button, comment-compose-stub) — the contextual modal IS the design, no full redirect happens. No code change. | `MISREAD` | — |
| QA-R5/MISREAD-D4 | Web/Discover | Note | Audit assumed "Near me" sort triggered `navigator.geolocation` and worried about the surprise permission dialog. Actually uses `sessionCityId` from the server session — no browser geo permission anywhere on the route. POLISH-B8 expanded the disabled-state tooltip to make the privacy posture explicit. | `MISREAD` | — |
| QA-R5/PUBLISH-001 | Web/Auth | Bug | Publish flow blocker: every `POST /api/publish/draft` returned 401 with `{"detail":"Missing Authorization header"}` for any user testing for >1 hour. **Root cause:** `ch_session` JWT (30d TTL) outlives `ch_access` (1h TTL); after 1h `apiFetch.readAccessToken()` returned null and no Bearer header was attached upstream. No web-side refresh flow existed despite `POST /api/v1/auth/refresh` being live in the API. Affects every authed server-side route handler, not just publish. **Fixed:** added `tryRefreshAccessToken()` in `api-client.ts` that reads `ch_refresh`, calls upstream refresh, sets new `ch_access` + `ch_refresh` cookies via `cookies().set()`, returns the new token. `apiFetch` now does a one-shot refresh + inline retry on any 401 (skipAuth=false, attempt=0), independent of the caller's retry budget so booking-style routes that pass `retries: 0` still self-heal. Symbol-on-jar guard prevents refresh loops if the refresh token itself is dead — falls through to the original 401 with an `api:refresh-unavailable` warn line so future debug is fast. | `FIXED` | P0 |
| QA-R5/PUBLISH-002 | Web/Publish | Bug | "Save failed · Retry" pill was opaque about cause — users had no way to tell network from server error from session expiry. **Fixed in `publish-wizard.tsx`:** 401 path now reads "Session expired — sign in to keep your draft, your changes won't be lost." Other failures surface the upstream `detail` field when present; otherwise hint at the most common cause ("Server hiccup — wait a moment and click Retry" for 5xx, "Couldn't save (HTTP X) — check your connection or sign in again" for other 4xx). | `FIXED` | P1 |
| QA-R5/PUBLISH-003 | Web/Publish | Bug | Publish button stayed active during autosave failure — clicking it on the Review step appeared to do nothing (or attempted to publish unsaved data). **Fixed:** `disabled={pending \|\| (step === 'review' && saveError !== null)}` blocks submission while there's an unresolved save error; tooltip "Resolve the save error before publishing" surfaces the gating reason on hover. | `FIXED` | P1 |
| QA-R5/PUBLISH-004 | Web/Publish | Enhancement | `/publish` type-selector cards felt unresponsive on first click in dev (cold-compile of the destination route). **Fixed:** added explicit `cursor: pointer` + `prefetch` + 120ms transition to the cards so the click affordance is immediate even before navigation resolves. Cold-compile delay itself is a dev-mode artifact and disappears in production. | `FIXED` | P3 |
| QA-R5/PUBLISH-005 | Web/Publish | Enhancement | Markdown body editor's "Write" tab confused users who typed `## ` and didn't see live formatting (TipTap auto-formats but only when the shortcut is at the start of a line followed by a space). **Fixed:** wizard's body-step hint now says "Click the toolbar buttons (H2 / H3 / B / I / quote / list) — or type the markdown shortcut at the start of a line followed by a space" with explicit `## ` `* ` `> ` examples. Sets the right expectation about both paths. | `FIXED` | P2 |
| QA-R5/PUBLISH-006 | Web/Publish | Note | "Preview requires a save first" — the live preview iframe needs a `draftId` (returned by autosave). When PUBLISH-001's auth bug killed every save, preview was permanently unavailable. **No code change**: the empty-state already renders a `<MarkdownBody>` fallback ("Save the draft once to enable the live preview. In the meantime, here's a markdown render:"). With auto-refresh fixing autosave, preview unlocks naturally on first save. | `INFO` | — |
| QA-R6/A1 | Web/Publish | Bug | Inline validation errors persisted until the user clicked Continue again — the field edit didn't clear them. **Fixed:** `update()` helper now calls `setError(null)` when there's an active error, so any keystroke / cover-image change drops the warning inline. | `FIXED` | P2 |
| QA-R6/A2 | Web/Publish | Bug | Publish button stayed active on the Review step even when required fields were missing. The `allReady` flag existed inside `<ReviewStep>` but never reached the parent's button gating. **Fixed:** ReviewStep now reports up via `onReadyChange`; the wizard's Publish button disables on `!reviewReady` with tooltip "Add the missing pieces above first." | `FIXED` | P1 |
| QA-R6/A3 | Web/Onboarding | Bug | `/onboarding/sub-categories` rendered raw upstream `detail` strings on submit failure — including "Missing Authorization header" when ch_access expired. **Fixed:** new `friendlySubmitError(status)` maps 401/403 → "Your session timed out. Refresh and try again.", 5xx → "Something went wrong on our end. Give it a moment and retry.", other 4xx → "Couldn't save — check your inputs and try again." Raw upstream detail no longer reaches users. | `FIXED` | P1 |
| QA-R6/A4 | Web/Header | Bug | `/onboarding/{welcome,sub-categories,city}` all passed `variant="auth"` to `<WebHeader>`, which forces the "Sign in" CTA regardless of session — the user was mid-onboarding (already authed) and saw a "Sign in" link in the header. **Fixed:** dropped `variant="auth"` and pass `session={session}` instead, letting the header's `resolvedVariant` logic show the logged-in chrome (profile menu + Saved + Bookings nav). The "auth" variant now lives only on `/signin`, `/signup`, `/forgot-password`. | `FIXED` | P1 |
| QA-R6/A5 | Web/Editor | Bug | TipTap toolbar buttons (B / I / H2 / H3 / quote / bullet / link / image / table / YouTube) had no hover tooltips — only screen-reader users got the labels via aria-label. **Fixed:** mirrored aria-label into `title` on `<ToolbarBtn>` so sighted users get the same disclosure on hover. | `FIXED` | P2 |
| QA-R6/A6 | Web/Publish | Enhancement | Step breadcrumbs (Cover → Details → Body → Review) were static `<span>`s — fixing a typo on the title from the Body step required Back, Back. **Fixed:** completed steps render as `<button>` with `onJumpToStep` callback so the user can leap back in one click. Future steps stay non-interactive. | `FIXED` | P3 |
| QA-R6/A7 | Web/Publish | Enhancement | Review-step checklist rows were static `<li>` elements — `○` next to "Story body" gave no path to fix it without scrolling and clicking Back. **Fixed:** each row is now a button mapping to its source step ('cover' / 'details' / 'body' / 'spots') via the same `onJumpToStep` callback as A6. Right-aligned "Edit →" / "Fix →" affordance per row. | `FIXED` | P3 |
| QA-R6/B1 | Web/Editor | Enhancement | Editor body was about 3 lines tall before scrolling — the `.ch-tiptap-prose` class had no `min-height`, just the 320px loading skeleton. Long-form writers felt cramped. **Fixed:** `.ch-tiptap-prose { min-height: 360px; padding: 16px }` in globals.css; matches the loading-state height. | `FIXED` | P2 |
| QA-R6/B2 | Web/Publish | Enhancement | Post and the three locked types (Itinerary / Experience / Event) on `/publish` looked identical except for opacity 0.7 — creators couldn't tell at a glance which to start with. **Fixed:** Post (and any future free type) gets a coral "FREE" chip top-right; locked cards get a grey "🔒 KYC" chip. Existing inline "Verify identity to unlock →" CTA inside locked cards is unchanged. | `FIXED` | P2 |
| QA-R6/B3 | Web/Publish | Copy | `/publish` eyebrow read "NEW" — confusing on a permanent screen ("new compared to what?"). **Fixed:** renamed to "Creator tools" matching the rest of the site's mono-eyebrow pattern. | `FIXED` | P3 |
| QA-R6/B4 | Web/Publish | Enhancement | Cover-crop step had two competing actions: "Use this crop" (inside the crop tool) + "Continue" (below it). Creators clicked Continue first, got the persistent "Add a cover photo" error, and got stuck. **Fixed:** auto-export the current crop on every pan/zoom (200ms debounced) so `coverDataUrl` is always fresh — dropped the inner "Use this crop" button entirely. Single Continue advances. | `FIXED` | P1 |
| QA-R6/B5 | Web/Studio | Enhancement | `/studio/content` empty state showed two competing primary CTAs: "+ New" in the header AND "Start publishing" in the EmptyState card. **Fixed:** the `actions` slot is omitted when `items.length === 0` so the empty state's CTA is the focal action. Header "+ New" returns once there's content. | `FIXED` | P3 |
| QA-R6/B6 | Web/Onboarding | Enhancement | The four sub-category cards used CSS gradient placeholders — no real travel imagery on a visual-storytelling platform's first impression. **Fixed:** replaced `<div className="ch-photo--konkan">` etc. with `<Image src={unsplashUrl}>` using verified-200 Unsplash IDs (open road, mountain pass, alpine forest, food spread). | `FIXED` | P3 |
| QA-R6/C1 | Web/Onboarding+Auth | Enhancement | Root cause for the "Welcome back, creator" placeholder + `@null` creator-profile handles: signup didn't require display name + username, both stayed null in the JWT/DB. **Fixed end-to-end:** new `/onboarding/profile` step (Step 1 of 3) collects both with client validation (display name 2-60 chars; username 3-20 chars, lowercase letters + numbers, lowercase-coerced + regex-stripped on input). Backed by new web route `/api/onboarding/profile` PUTting to upstream `/api/v1/users/me` with retries=1 (409 → "That handle is taken"). `/api/auth/signin`'s `onboardingComplete` gate now also requires `display_name >= 2` AND `username >= 3` so first-time users land on the profile step before sub-categories. Backstop coral nudge banner on `/studio` and `/you` for existing users with null fields, linking to `/onboarding/profile`. | `FIXED` | P1 |
| QA-R6/MISREAD-D1 | Web/Publish | Note | Audit reported "Clicking the card body does nothing — only the CTA navigates." Actually the entire `<Link>` wraps the card content (per [apps/web/src/app/publish/page.tsx:99-143](apps/web/src/app/publish/page.tsx#L99-L143)) and clicks anywhere navigate. The "doesn't navigate" experience matches the dev-mode cold-compile delay already addressed in QA-R5/PUBLISH-004 (cursor + transition for click affordance). | `MISREAD` | — |
| QA-R6/MISREAD-D2 | Web/Publish | Note | Audit reported "Save failed gives no actionable detail." Already detail-aware in `70e388e` — 401 → "Session expired, sign in to keep your draft", 5xx → "Server hiccup", 4xx → status code + connection hint. Audit hit a stale build. | `MISREAD` | — |
| QA-R6/MISREAD-D3 | Web/Publish | Note | Audit reported "Missing Authorization header raw error visible in publish wizard." Already auto-recovers via `apiFetch`'s refresh-on-401 (`70e388e`) + half-baked-session cleanup (`4d5118c`). The wizard's 401 path now shows "Session expired — sign in to keep your draft" instead of raw upstream text. | `MISREAD` | — |
| QA-R6/MISREAD-D4 | Web/KYC | Note | Audit reported KYC "Start verification" doesn't begin the wizard. Actually the button correctly hrefs `/studio/kyc/submit` which renders `<KycWizard>` directly with Step 1. The "stayed on overview" experience was almost certainly the same `variant="auth"` header bug as QA-R6/A4 — the `/studio/kyc/submit` page had a "Sign in" link in the header that made the user think they hadn't moved. Fixing A4 also fixes this. | `MISREAD` | — |
| M2.5/PERF-001 | Web/Performance | Enhancement | **LCP + FCP perf pass (2026-05-02 → 2026-05-03)** addressing the founder-reported "hero pop-in" on `/` and `/content/[id]`. **Six-item plan, five shipped:** (1) `<ChapterHero>` CSS `background-image` → `<Image fill priority sizes>` so the cover enters the browser preload graph synchronously with FCP — the dominant LCP win; (2) `<ParallaxHero>` same swap on both reduced-motion and animated branches, parallax preserved on the wrapper div; (3) home page split into `<HomeHero>` (awaits only `getFeaturedChapterStory`) + `<Suspense fallback={<HomeFeedSkeleton/>}><HomeFeed/></Suspense>` (9-section fan-out + cities + spotlight + 11 rails). `getFeaturedChapterStory`, `getHomeFeedSections`, and `fetchQuestSummary` wrapped with `React.cache()` for per-request dedup so hero + feed siblings share results. Also short-circuited `getFeaturedChapterStory`'s candidate detail loop from serial → `Promise.all` over capped 6-pool; (5) `<SignInModal>` body code-split via `React.lazy()` + `<Suspense fallback={null}>` so the modal's framer-motion + form code is excluded from every initial route bundle (only loaded on first `openSignInModal()`); (6) `@next/bundle-analyzer` wired behind `ANALYZE=true pnpm build` for ongoing visibility. **Item 4 (`framer-motion` → `motion/react` granular imports) deferred as false flag** — `framer-motion@12` doesn't expose `motion/react` as an entry point; only `framer-motion`, `framer-motion/client`, `framer-motion/m`, `framer-motion/mini` are available; `optimizePackageImports: ['framer-motion']` already in `next.config.ts` handles tree-shaking. **Verification:** `pnpm typecheck` 0 errors; `pnpm lint` 0 errors; `pnpm test` 352/352 pass; `pnpm build` clean — `/` route 7.39 kB + 162 kB First Load JS, well under WEB-NFR-004 budget. Lighthouse mobile profile re-run pending dev-server restart (the user's still-running dev server has a stale `.next/` cache that's flooding `Cannot find module './2461.js'` errors and should be cleaned with `rm -rf apps/web/.next && pnpm dev` before measuring). | `FIXED` | P1 |
